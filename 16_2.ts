...
// const fileContents: string[] = _.split(fs.readFileSync("test_input", {encoding: "utf8"}).trim(), ",");
const fileContents: string[] = _.split(fs.readFileSync("inputs/16_1", {encoding: "utf8"}).trim(), ",");

// let programs: string[] = _.map(_.range(65, 65 + 5), (value: number): string => {
let programs: string[] = _.map(_.range(65, 65 + 16), (value: number): string => {
    return String.fromCharCode(value);
});

// Kind of cheating here, since we saw online that cycles existed. Whoops
const alreadySeen: string[][] = [programs.slice()];

_.forEach(_.range(0, 1000), (): boolean => {
    ...
    // Can't just compare arrays directly. Need to do it manually.
    let cycleFound: boolean = true;
    _.forEach(programs, (value: string, index: number, collection: string[]) => {
        if (value !== alreadySeen[0][index]) {
            cycleFound = false;
        }
    });

    if (cycleFound) {
        // We've found a cycle. Yay.
        return false;
    }

    alreadySeen.push(programs.slice());
    return true;

});

console.log(alreadySeen.length);

console.log(_.join(alreadySeen[1000000000 % alreadySeen.length], ""));
